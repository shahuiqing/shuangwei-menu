...
export async function getPurchaseOrders(): Promise<PurchaseOrder[]> {
  try {
    ...
    handleSupabaseReadError(e, "getPurchaseOrders");
  }
...
export async function savePurchaseOrder(po: PurchaseOrder) {
  const full: PurchaseOrder = {
    ...
    total_cost: Number(
      (Number(po.quantity) * Number(po.unit_price)).toFixed(2),
    ),
    created_at: po.created_at || new Date().toISOString(),
  };
  const parsed = purchaseOrderSchema.safeParse(full);
  if (!parsed.success)
    logger.warn("finance", "purchase zod", parsed.error.issues.slice(0, 2));
...
export async function deletePurchaseOrder(id: string) {
  try {
    ...
    handleSupabaseWriteError(e, "deletePurchaseOrder");
  }
  lsSet(
    "local_purchase_orders",
    lsGet<PurchaseOrder[]>("local_purchase_orders", []).filter(
      (x) => x.id !== id,
    ),
  );
  triggerBroadcast("inventory_changed");
...
> {
  try {
    ...
    handleSupabaseReadError(e, "getInventoryTransactions");
  }
...
export async function addInventoryTransaction(tx: InventoryTransaction) {
  ...
  if (!parsed.success)
    logger.warn("finance", "tx zod", parsed.error.issues.slice(0, 2));
  ...
  try {
    ...
    handleSupabaseWriteError(e, "addInventoryTransaction");
  }
...
// 计算某日期区间内的采购总额（按采购日期）
async function purchasesTotalByDate(
  from: string,
  to: string,
...
export async function getWasteAnalysis(
    ...
    (o.items || []).forEach((oi: any) => {
      ...
      matched.forEach((bom) => {
        theoretical.set(
          ...
          (theoretical.get(bom.inventory_item_id) || 0) +
            Number(bom.dosage) * qty,
        );
      });
    });
  });
  ...
  pos.forEach((p) => {
    ...
    purchasesQty.set(
      p.item_id,
      (purchasesQty.get(p.item_id) || 0) + Number(p.quantity || 0),
    );
  });
  ...
  txs.forEach((tx) => {
    ...
    if (d < periodStart) {
      ...
      opening.set(
        tx.item_id,
        (opening.get(tx.item_id) || 0) + (isIn ? qty : -qty),
      );
    } else if (d > periodEnd) {
      // 周期后发生，不影响
    } else {
      ...
      closing.set(
        tx.item_id,
        (closing.get(tx.item_id) || 0) + (isIn ? qty : -qty),
      );
    }
  });
  ...
  inv.forEach((i) => {
    ...
    const wasteRate =
      actual > 0 ? Number(((wasteQty / actual) * 100).toFixed(1)) : 0;
...
export async function getPeriodTotals(
  ...
  const wasteRate =
    foodCost > 0 ? Number(((wasteCost / foodCost) * 100).toFixed(1)) : 0;
  return {
