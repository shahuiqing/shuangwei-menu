...
import { compressImage, compressBase64Image } from "../../utils/image";
...
export function useMenuManager(deps: MenuManagerDeps) {
  ...
  const [newCategory, setNewCategory] = useState({
    name: "", enName: "", frName: "", arName: "", maName: "",
  });
  const [newDish, setNewDish] = useState({
    title: "", enTitle: "", frTitle: "", arTitle: "", maTitle: "",
    description: "", enDescription: "", frDescription: "", arDescription: "", maDescription: "",
    price: "", image: "", allergens: [] as string[], stock: "",
  });
  const [uploadingItemId, setUploadingItemId] = useState<string | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [promptDialog, setPromptDialog] = useState<{
    isOpen: boolean; message: string; defaultValue: string; onConfirm: (value: string) => void;
  } | null>(null);
  ...
  const handleMoveDish = (catId: string, itemIdx: number, direction: 'up' | 'down' | 'top' | 'bottom') => {
    const updatedCategories = categories.map((cat) => {
      ...
      if (targetIdx < 0 || targetIdx >= items.length || targetIdx === itemIdx) return cat;
      const [movedItem] = items.splice(itemIdx, 1);
      items.splice(targetIdx, 0, movedItem);
      return { ...cat, items };
    });
    setCategories(updatedCategories);
    if (onSaveToCloud) onSaveToCloud({ categories: updatedCategories, silent: true });
  };
  ...
  const base64Count = useMemo(() => {
    ...
    categories.forEach((cat: any) => {
      (cat.items || []).forEach((item: any) => {
        if (item.image && item.image.startsWith("data:image/") && item.image.length > 15000) count++;
      });
    });
    return count;
  }, [categories]);
  ...
  const handleExportJson = () => {
    try {
      const backupObj = {
        categories, promotions, restaurantName, welcomeMessage,
        bgUrl, logoUrl, layoutStyle, theme, soundEnabled,
        adminPassword, devicePasswords, securityQuestion, securityAnswer,
      ...
      setTimeout(() => {
        if (downloadAnchorNode.parentNode) downloadAnchorNode.parentNode.removeChild(downloadAnchorNode);
  ...
  const handleExportCsv = () => {
    try {
      const headers = [
        "分类ID", "分类名称(中文)", "分类英文名", "分类法文名",
        "菜品ID", "菜品中文名", "英文名", "法文名", "阿拉伯文名", "摩洛哥文名",
        "价格", "中文描述", "英文描述", "法文描述", "阿拉伯文描述", "摩洛哥文描述", "图片URL",
      ];
      ...
      categories.forEach((cat: any) => {
        ...
        (cat.items || []).forEach((item: any) => {
          rows.push([
            cat.id || "", catName, catEnName, catFrName,
            item.id || "", item.title || "", item.enTitle || "", item.frTitle || "",
            item.arTitle || "", item.maTitle || "", item.price || "",
            item.description || "", item.enDescription || "", item.frDescription || "",
            item.arDescription || "", item.maDescription || "",
  ...
  const handleAIEnhanceImage = async () => {
    ...
    try {
      ...
      const mimeType = newDish.image.split(";")[0]?.split(":")[1] ?? "image/jpeg";
  ...
  const handleAddCategory = (e: FormEvent) => {
    ...
    const updatedCategories = [...categories, {
      id: catId, name: newCategory.name, enName: newCategory.enName,
      frName: newCategory.frName, arName: newCategory.arName, maName: newCategory.maName, items: [],
    }];
    setCategories(updatedCategories);
    setNewCategory({ name: "", enName: "", frName: "", arName: "", maName: "" });
  ...
  const handleDeleteCategory = (idToRemove: string) => {
    ...
    const catTitle = catToRemove?.name || catToRemove?.title || "未命名分类";
    setConfirmDialog({
      isOpen: true, title: "删除分类确认 (1/2)", stepBadge: "第一级确认",
      message: `确定要删除分类【${catTitle}】及其所有关联菜品吗？`,
      subDetail: "点击“下一步”将进入第二级危险确认阶段，请再次核对删除范围。",
      confirmText: "下一步 (进入二次确认) →",
      confirmBtnClass: "px-4 py-2 text-sm font-semibold bg-amber-600 hover:bg-amber-500 text-white rounded-xl transition-colors shadow-lg shadow-amber-600/20",
      onConfirm: () => {
        setConfirmDialog({
          isOpen: true, title: "⚠️ 强制彻底删除分类 (2/2)", stepBadge: "第二级确认 (不可撤销)",
          ...
          confirmBtnClass: "px-4 py-2 text-sm font-bold bg-red-600 hover:bg-red-700 text-white rounded-xl transition-colors shadow-lg shadow-red-600/30 ring-2 ring-red-400/50 animate-pulse",
          onConfirm: () => {
            const updatedCategories = categories.filter((c: any) => c.id !== idToRemove);
            setCategories(updatedCategories);
            const newDeletedIds = Array.from(new Set([...(deletedItemIds || [])]));
            if (idToRemove && !newDeletedIds.includes(idToRemove)) newDeletedIds.push(idToRemove);
            if (catToRemove) {
              if (catToRemove.name && !newDeletedIds.includes(catToRemove.name)) newDeletedIds.push(catToRemove.name);
              if (catToRemove.title && !newDeletedIds.includes(catToRemove.title)) newDeletedIds.push(catToRemove.title);
              (catToRemove.items || []).forEach((item: any) => {
                if (item.id && !newDeletedIds.includes(item.id)) newDeletedIds.push(item.id);
                if (item.title && !newDeletedIds.includes(item.title)) newDeletedIds.push(item.title);
                if (item.name && !newDeletedIds.includes(item.name)) newDeletedIds.push(item.name);
              });
            }
            if (setDeletedItemIds) setDeletedItemIds(newDeletedIds);
            if (onSaveToCloud) onSaveToCloud({ categories: updatedCategories, deletedItemIds: newDeletedIds, silent: true });
            api.triggerBroadcast("dish_deleted", { categoryId: idToRemove, catTitle, deletedItemIds: newDeletedIds, timestamp: new Date().toISOString() });
            api.triggerBroadcast("settings_changed", { deletedItemIds: newDeletedIds });
            if (selectedCategory === idToRemove) setSelectedCategory(updatedCategories[0]?.id || "");
  ...
  const handleDeleteDish = (categoryId: string, dishId: string) => {
    ...
    const itemToDelete = (cat?.items || []).find((item: any) => item.id === dishId);
    const dishTitle = itemToDelete?.title || itemToDelete?.name || "未知菜品";
    setConfirmDialog({
      isOpen: true, title: "删除菜品确认 (1/2)", stepBadge: "第一级确认",
      ...
      onConfirm: () => {
        setConfirmDialog({
          isOpen: true, title: "⚠️ 强制彻底删除菜品 (2/2)", stepBadge: "第二级确认 (不可撤销)",
          ...
          confirmBtnClass: "px-4 py-2 text-sm font-bold bg-red-600 hover:bg-red-700 text-white rounded-xl transition-colors shadow-lg shadow-red-600/30 ring-2 ring-red-400/50 animate-pulse",
          onConfirm: () => {
            let deletedTitle = dishTitle;
            let deletedName = itemToDelete?.name || "";
            const updatedCategories = categories.map((cat: any) => {
              if (cat.id === categoryId) return { ...cat, items: (cat.items || []).filter((item: any) => item.id !== dishId) };
              return cat;
            });
            setCategories(updatedCategories);
            const newDeletedIds = Array.from(new Set([...(deletedItemIds || [])]));
            if (dishId && !newDeletedIds.includes(dishId)) newDeletedIds.push(dishId);
            if (deletedTitle && !newDeletedIds.includes(deletedTitle)) newDeletedIds.push(deletedTitle);
            if (deletedName && !newDeletedIds.includes(deletedName)) newDeletedIds.push(deletedName);
            if (setDeletedItemIds) setDeletedItemIds(newDeletedIds);
            if (onSaveToCloud) onSaveToCloud({ categories: updatedCategories, deletedItemIds: newDeletedIds, silent: true });
            api.triggerBroadcast("dish_deleted", { dishId, dishTitle, deletedItemIds: newDeletedIds, timestamp: new Date().toISOString() });
            api.triggerBroadcast("settings_changed", { deletedItemIds: newDeletedIds });
            setConfirmDialog(null);
          },
        });
  ...
  const handleAddDish = (e: FormEvent) => {
    e.preventDefault();
    if (!selectedCategory || !newDish.title || !newDish.image || !newDish.price) return;
    const finalPrice = currency && currency !== "none" ? `${newDish.price} ${currency}` : newDish.price;
    const updatedCategories = categories.map((cat: any) => {
      if (cat.id === selectedCategory) {
        return {
          ...cat,
          items: [...cat.items, {
            ...newDish, price: finalPrice,
