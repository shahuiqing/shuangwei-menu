...
export default function DiagnosticModal({
  ...
  >("idle");
  const [stepDb, setStepDb] = useState<
    "idle" | "running" | "success" | "error"
  >("idle");
  const [stepRealtime, setStepRealtime] = useState<
    "idle" | "running" | "success" | "error"
  >("idle");
  ...
  const [receivedBroadcasts, setReceivedBroadcasts] = useState<
    Array<{ sender: string; time: string; delay: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    addLog(
      `初始化诊断广播监听器 / Initializing diagnostic broadcast listener...`,
    );
  ...
  const runDiagnostics = async () => {
    ...
    if (!url || !key) {
      setStepConfig("error");
      addLog(
        `❌ 环境配置检查失败: VITE_SUPABASE_URL 或 VITE_SUPABASE_ANON_KEY 未设置！`,
      );
      addLog(
        `💡 解决方案: 请检查系统根目录下是否存在正确的 .env 配置文件，并保存最新配置。`,
      );
      return;
    }
    ...
    try {
      ...
      addLog(
        `🏠 餐厅名称 / Restaurant: ${settingsData.restaurantName || "未设置"}`,
      );
      addLog(
        `📂 菜品分类数 / Categories count: ${settingsData.categories?.length || 0}`,
      );
    } catch (err: any) {
      setStepDb("error");
      addLog(
        `❌ 数据库连接或查询失败 / Database read failed: ${err.message || err}`,
      );
      addLog(
        `💡 提示: 请检查本地网络、数据库防火墙规则或 Supabase 项目健康状态。`,
      );
      return;
    }
    ...
    addLog(
      `🌐 正在初始化 WebSocket 实时回环测试 / Testing Realtime Loopback...`,
    );
    ...
    try {
      ...
      const loopbackChannel = sb.channel(loopbackChannelId, {
        config: {
          broadcast: { self: true },
        },
      });
      ...
      loopbackChannel.on("broadcast", { event: "loopback-ping" }, () => {
        loopbackReceived = true;
        addLog(
          `🔄 回环测试成功! 收到自身广播信号 / Loopback received successfully!`,
        );
      });

      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          sb.removeChannel(loopbackChannel);
          reject(
            new Error("Timeout (3s) waiting for realtime loopback response"),
          );
        }, 3000);
        ...
        loopbackChannel.subscribe((status) => {
          if (status === "SUBSCRIBED") {
            addLog(
              `📡 实时信道已连接，发送 Ping 信号 / Channel subscribed, sending Ping...`,
            );
            loopbackChannel
              .send({
                type: "broadcast",
              ...
              .then(() => {
                ...
                setTimeout(() => {
                  ...
                  if (loopbackReceived) {
                    ...
                    reject(
                      new Error(
                        "Ping sent but no loopback received. Realtime might be disabled.",
                      ),
                    );
                  }
                }, 800);
            ...
            reject(
              new Error(
                "Supabase Realtime server rejected the connection (CHANNEL_ERROR)",
              ),
            );
          }
        });
      ...
      addLog(
        `💡 电脑端浏览器或本地网络环境可能限制了 WebSocket 信道，但【请不用担心】！`,
      );
      addLog(
        `📢 系统已自动启动「高频 HTTP 混合轮询同步引擎 (Hybrid Polling Engine)」！`,
      );
      addLog(
        `⚡ 所有菜单设置、餐桌状态和顾客订单，将继续以 3秒/次 的超高频率在后台静默轮询同步！`,
      );
      addLog(`✅ 电脑端与手机端数据依然保持同步与可用！`);
      addLog(
        `💡 提示: 如果想恢复最完美的毫秒级 WebSocket，可尝试强刷电脑端 (Ctrl+F5 或 Shift+Command+R) 或检查网络代理设置。`,
      );
    }
  };
  ...
  const handleSendInteractiveBroadcast = async () => {
    ...
    try {
      ...
      if (!channel) {
        ...
        channel = sb.channel("diagnostics-interactive", {
      ...
      addLog(
        `✅ 广播发送成功！如果你在另一台设备(如手机/电脑)上打开了本面板，它将立刻收到该消息！`,
      );
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col h-[90vh] max-h-[750px]">
          ...
          <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-4 flex items-center justify-between">
            ...
            <div className="flex flex-col items-end">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                />
                <span
                  className={`text-xs font-bold ${realtimeStatus === "SUBSCRIBED" ? "text-green-400" : "text-yellow-400"}`}
                >
                  {realtimeStatus === "SUBSCRIBED"
                    ? "已连接 / Connected"
                    : realtimeStatus}
            ...
            <div className="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-3 space-y-3.5">
              ...
              <div className="flex items-center justify-between border-t border-zinc-900/80 pt-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-zinc-200">
                      2. 数据库 API 读取 (REST Read)
                    </p>
                    <p className="text-[10px] text-zinc-500">
                      向 Supabase 发起请求，确保拉取菜单/设置一切正常
                    </p>
                  </div>
                </div>
                <div>
              ...
              <div className="flex items-center justify-between border-t border-zinc-900/80 pt-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-zinc-200">
                      3. WebSocket 广播回环 (WS Loopback)
                    </p>
                    <p className="text-[10px] text-zinc-500">
                      发送实时广播并检测自身接收，验证收发链路双向通畅
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  {stepRealtime === "running" && (
                    <Loader2
                      className="animate-spin text-orange-500"
                      size={14}
                    />
                  )}
                  {stepRealtime === "success" && (
                    <CheckCircle2 className="text-green-500" size={15} />
                  )}
                  {stepRealtime === "error" && (
                    <AlertCircle className="text-red-500" size={15} />
          ...
          <div className="space-y-3">
            ...
            <div className="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-4 space-y-4">
              <p className="text-xs text-zinc-400 leading-relaxed">
                同时在手机和电脑端打开此诊断面板，在其中一个端点击下方
                <strong>“发送广播测试”</strong>
                ，正常状态下，所有已打开此面板的端都会<strong>立刻</strong>
                显示收到了对方发送的消息：
              </p>
            ...
            <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-3 h-32 overflow-y-auto font-mono text-[10px] text-zinc-400 space-y-1 scrollbar-thin scrollbar-thumb-zinc-800">
              {logs.length === 0 ? (
                <p className="text-zinc-600 italic">
                  点击上方 “一键检测” 即可生成实时诊断报告 / Click test button
                  to start
                </p>
              ) : (
                logs.map((log, i) => (
                  <p
                    ...
                    className={
                      log.includes("❌")
                        ? "text-red-400"
                        : log.includes("✅")
                          ? "text-green-400 animate-pulse-once"
                          : log.includes("📢")
                            ? "text-orange-400 font-bold bg-orange-950/10 px-1 py-0.5 rounded"
                            : "text-zinc-400"
                    }
                  >
                    {log}
        ...
        <div className="p-4 bg-zinc-900/30 border-t border-zinc-900/80 flex flex-col gap-2">
          <p className="text-[10px] text-zinc-500 text-center leading-relaxed">
            * 电脑网页无法同步时，请尝试在电脑上按 <strong>Ctrl+F5</strong>{" "}
            (Windows) 或 <strong>Shift+Command+R</strong> (Mac)
            彻底刷新页面，以拉取最新的环境变量及缓存。
          </p>
        </div>
      </div>
    </div>
