...
import type { MenuCategory, Promotion, AppSettings } from "../types/menu";
import {
...
export const useMenuStore = create<MenuState>((set, get) => ({
  categories: (() => {
    ...
    const ver = safeGetItem("menuVersion");
    if (saved && ver === "2.0") {
      try {
        const parsed = JSON.parse(saved);
        const del = JSON.parse(safeGetItem("menuDeletedItemIds") || "[]");
  ...
  deletedItemIds: (() => {
    try {
      return JSON.parse(safeGetItem("menuDeletedItemIds") || "[]");
    } catch {
      return [];
    }
  })(),
  promotions: (() => {
    try {
      return JSON.parse(safeGetItem("menuPromotions") || "[]");
    } catch {
      return [];
    }
  })(),
  ...
  setFromSettings: (s) => {
    if (s.categories)
      set({
        categories: mergeAndOrderCategories(
          s.categories as any,
          INITIAL_MENU_CATEGORIES,
          s.deletedItemIds || [],
        ) as any,
    ...
    if (s.restaurantName !== undefined)
      set({ restaurantName: s.restaurantName });
    if (s.welcomeMessage !== undefined)
      set({ welcomeMessage: s.welcomeMessage });
