...
function isSafePrintUrl(u: string): boolean {
  try{
    ...
    if(/^10\./.test(url.hostname) || /^192\.168\./.test(url.hostname) || /^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(url.hostname)) return false;
    // 额外白名单：可配 PRINT_WORKER_ALLOWLIST=host1,host2
    const allow=(process.env.PRINT_WORKER_ALLOWLIST||'').split(',').map(s=>s.trim()).filter(Boolean);
    if(allow.length && !allow.includes(url.hostname) && !url.hostname.endsWith('.workers.dev')) return false;
    return true;
  } catch{ return false; }
}
// P0-5 原子写
...
app.use(cors({
  origin: (origin, cb)=>{
    ...
    if(ALLOWED_ORIGINS.length===0){
      if(isProdCors) return cb(new Error('CORS blocked: CORS_ORIGINS not configured'), false);
      return cb(null, true);
    }
...
app.post('/api/init-db', requireAdmin, async (req, res) => {
  try {
    if (CF_API_TOKEN && CF_ACCOUNT_ID && CF_D1_DB_ID) {
      const result = await queryD1("CREATE TABLE IF NOT EXISTS settings (id TEXT PRIMARY KEY, data TEXT)");
...
app.get('/api/settings', async (req, res) => {
  try {
    if (CF_API_TOKEN && CF_ACCOUNT_ID && CF_D1_DB_ID) {
      const result = await queryD1("SELECT data FROM settings WHERE id = ?", ['global']);
...
const LOCAL_ORDERS_FILE = path.join(process.env.NODE_ENV === 'production' ? '/tmp' : process.cwd(), 'local-orders.json');
if (process.env.NODE_ENV === 'production' && !fs.existsSync(LOCAL_ORDERS_FILE)) {
...
app.post('/api/orders', rateLimit(60_000, 20), async (req, res) => {
  try {
    ...
    const printWorkerUrl = process.env.PRINT_WORKER_URL || "https://dayin.shahuiqing2024.workers.dev/";
    const printWorkerToken = process.env.PRINT_WORKER_TOKEN;
    if (printWorkerUrl) {
      if(!isSafePrintUrl(printWorkerUrl)){
        console.warn(`[security] blocked unsafe PRINT_WORKER_URL: ${printWorkerUrl}`);
      } else {
        try {
          const headers: Record<string, string> = { "Content-Type": "application/json" };
          if (printWorkerToken) headers["Authorization"] = `Bearer ${printWorkerToken}`;
          fetch(printWorkerUrl, {
            method: "POST",
            headers,
            body: JSON.stringify(payload)
          }).catch(err => console.error("Error sending to external worker:", err));
        } catch (e) {
    ...
    res.json({ success: true, message: 'Order received successfully', order: payload });
  } catch (error: any) {
...
app.post('/api/webhooks/orders', rateLimit(60_000, 30), async (req, res) => {
  const expectedToken = process.env.PRINT_WORKER_TOKEN || process.env.ADMIN_SECRET;
  if (expectedToken) {
    const got = (req.headers['x-webhook-token'] as string) || (req.headers['authorization'] as string)?.replace('Bearer ', '') || (req.body as any)?.token;
...
app.post('/api/edgeone-kv/put', requireAdmin, async (req, res) => {
  try {
    ...
    const globalKv = (globalThis as any).my_kv || (globalThis as any).env?.my_kv;
    if (globalKv && typeof globalKv.put === 'function') {
      await globalKv.put(key, typeof value === 'object' ? JSON.stringify(value) : String(value));
    ...
    try {
      ...
      const storageDir = path.join(process.env.NODE_ENV === 'production' ? '/tmp' : process.cwd(), 'kv-data');
      if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
      const safeKey = String(key).replace(/[/\\?%*:|"<>]/g, '_');
      writeFileAtomic(path.join(storageDir, safeKey), typeof value === 'object' ? JSON.stringify(value) : String(value));
    }
...
app.get('/api/edgeone-kv/get', async (req, res) => {
  try {
    ...
    if (globalKv && typeof globalKv.get === 'function') {
      const val = await globalKv.get(key, type === 'json' ? { type: 'json' } : 'text');
      return res.json({ success: true, value: val, source: 'edgeone-native-kv' });
    }
    ...
    try {
      ...
      if (type === 'json' && typeof content === 'string') {
        try { val = JSON.parse(content); } catch {}
      }
...
app.get('/api/edgeone-kv/list', async (req, res) => {
  try {
    ...
    if (globalKv && typeof globalKv.list === 'function') {
      ...
      return res.json({ success: true, ...listRes, source: 'edgeone-native-kv' });
    }

    // 本地存储目录遍历
    const storageDir = path.join(process.env.NODE_ENV === 'production' ? '/tmp' : process.cwd(), 'kv-data');
    if (!fs.existsSync(storageDir)) {
      return res.json({ success: true, complete: true, cursor: null, keys: [] });
    }
...
app.post('/api/edgeone-blob/set', requireAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      const storageDir = path.join(process.env.NODE_ENV === 'production' ? '/tmp' : process.cwd(), 'blob-data');
      if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
      const safeKey = String(key).replace(/[/\\?%*:|"<>]/g, '_');
      writeFileAtomic(path.join(storageDir, safeKey), typeof value === 'object' ? JSON.stringify(value) : String(value));
    }
...
app.get('/api/edgeone-blob/get', async (req, res) => {
  try {
    ...
    try {
      const { getStore } = await import('@edgeone/pages-blob');
      const store = getStore(storeName);
      const content = await store.get(key, consistency === 'strong' ? { consistency: 'strong' } as any : undefined);
...
async function startServer() {
    ...
    ws.on('message', async (message) => {
      try {
        ...
        if (data.type === 'sync-cart') {
          ...
          cartHub.broadcastAdmin({ message: data.message, notifyType: data.notifyType, table: data.table, action: data.action });
        }
  ...
  let lastServerMoroccoClearDate = '';

  function getMoroccoTimeInfo() {
    const now = new Date();
